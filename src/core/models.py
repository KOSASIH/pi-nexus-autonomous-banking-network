...
class BankAccount(Base):
    ...
    def to_dict(self):
        return {'id': self.id, 'account_number': self.account_number, 'account_holder': self.account_holder}

def create_bank_account(session, account_number, account_holder):
    new_account = BankAccount(account_number=account_number, account_holder=account_holder)
...
def get_bank_accounts_by_query(session, query):
    return session.query(BankAccount).filter(BankAccount.account_number.ilike(f"%{query}%") | BankAccount.account_holder.ilike(f"%{query}%")).all()
