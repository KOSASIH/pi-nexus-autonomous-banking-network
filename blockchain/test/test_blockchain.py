...
class TestBlockchain(unittest.TestCase):
    ...
    def test_calculate_hash(self):
        ...
        block.data = 'New Data'
        block.hash = None
        calculated_hash = self.blockchain.calculate_hash(block.index, block.previous_hash, block.timestamp, block.data)
    ...
    def test_is_chain_valid(self):
        ...
        invalid_chain.append(Block(len(invalid_chain), invalid_chain[-1].hash, int(time.time()), 'Invalid Data', None))
    ...
    def test_replace_chain(self):
        ...
        valid_chain.append(Block(len(valid_chain), valid_chain[-1].hash, int(time.time()), 'Valid Data', None))
