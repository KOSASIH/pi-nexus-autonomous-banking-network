...
class Ethereum:
    ...
    def send_transaction_to_contract(
        self, contract_instance, function_name, *args, gas=None, gas_price=None
    ):
        """
        ...
        tx = function.buildTransaction(
            {"from": self.web3.eth.defaultAccount, "gas": gas, "gasPrice": gas_price}
        )
        signed_tx = self.web3.eth.account.signTransaction(
            tx, private_key=self.web3.eth.defaultAccount.privateKey.key
        )
