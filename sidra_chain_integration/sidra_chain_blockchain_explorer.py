...
@app.callback(Output('block-graph', 'figure'),
              [Input('block-number-input', 'value'),
...
def update_graph(block_number, n):
    ...
    fig = go.Figure(data=[go.Bar(x=block_info['transactions'], y=block_info['transaction_values'])])
    return fig
