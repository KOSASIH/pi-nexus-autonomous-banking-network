...
class SidraChain:
    ...
    def create_account(self):
        ...
        response = requests.post(f'https://{self.network}.sidra.chain/api/v1/accounts', 
                                 headers={'Authorization': f'Bearer {self.api_key}'})
        return response.json()

    def create_smart_contract(self, contract_name, functions):
        # Create a new smart contract on the Sidra Chain network
        response = requests.post(f'https://{self.network}.sidra.chain/api/v1/contracts', 
                                 headers={'Authorization': f'Bearer {self.api_key}'},
    ...
    def deploy_smart_contract(self, contract_name):
        ...
        response = requests.post(f'https://{self.network}.sidra.chain/api/v1/contracts/{contract_name}/deploy', 
                                 headers={'Authorization': f'Bearer {self.api_key}'})
        return response.json()
