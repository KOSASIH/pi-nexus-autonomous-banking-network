...
def get_sidra_chain():
    ...
    return jsonify([{"id": sc.id, "chain_id": sc.chain_id, "chain_name": sc.chain_name} for sc in sidra_chain])

@app.route("/sidra_chain", methods=["POST"])
def create_sidra_chain():
    data = request.get_json()
    sidra_chain = SidraChainModel(chain_id=data["chain_id"], chain_name=data["chain_name"])
