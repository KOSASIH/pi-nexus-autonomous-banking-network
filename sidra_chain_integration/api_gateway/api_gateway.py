...
class LoanProcessingAPI(Resource):
    def post(self):
        ...
        response = requests.post(
            f'https://{os.environ["SIDRA_CHAIN_NETWORK"]}.sidra.chain/api/v1/contracts/LoanProcessingContract/evaluateLoanApplication',
...
class InvestmentManagementAPI(Resource):
    def get(self):
        ...
        response = requests.get(
            f'https://{os.environ["SIDRA_CHAIN_NETWORK"]}.sidra.chain/api/v1/contracts/InvestmentManagementContract/analyzeMarketTrends',
            headers={"Authorization": f'Bearer {os.environ["SIDRA_CHAIN_API_KEY"]}'},
        )
        return response.json()
...
class RiskAssessmentAPI(Resource):
    def post(self):
        ...
        response = requests.post(
            f'https://{os.environ["SIDRA_CHAIN_NETWORK"]}.sidra.chain/api/v1/contracts/RiskAssessmentContract/identifyRisks',
            headers={"Authorization": f'Bearer {os.environ["SIDRA_CHAIN_API_KEY"]}'},
            json=request.get_json(),
        )
        return response.json()
