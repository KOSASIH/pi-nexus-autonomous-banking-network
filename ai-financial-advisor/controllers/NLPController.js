...
router.post('/analyzeText', async (req, res) => {
  ...
  const financialEntities = entities.filter(entity => entity.type === 'MONEY' || entity.type === 'DATE');
  ...
  const financialAdvisor = await financialAdvisorModel.findOne({ userId: req.user.id });
  const advice = financialAdvisor.generateAdvice(financialData, intent, sentimentAnalysis);
...
router.post('/getFinancialGoal', async (req, res) => {
  ...
  const goalEntities = entities.filter(entity => entity.type === 'FINANCIAL_GOAL');
