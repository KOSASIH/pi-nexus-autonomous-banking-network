...
class Block:
    def __init__(
        self, index: int, previous_hash: str, timestamp: float, data: Dict, hash: str
    ):
        self.index = index
...
class Blockchain:
    ...
    def create_genesis_block(self) -> Block:
        return Block(
            ...
            self.calculate_hash(
                0, "0" * 64, int(time.time()), {"transaction_data": "Genesis Block"}
            ),
        )

    def calculate_hash(
        self, index: int, previous_hash: str, timestamp: float, data: Dict
    ) -> str:
        value = (
            str(index)
            + previous_hash
