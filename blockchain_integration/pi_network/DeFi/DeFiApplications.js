...
const compoundLendingPool = new Compound(
  "0xCOMPOUND_LENDING_POOL_ADDRESS",
  web3,
);
...
async function DeFiApp() {
  ...
  async function lendTokens(amount, interestRate) {
    ...
    console.log(
      `Lending balance: ${await aaveLendingPool.getUserBalance(wallet.address).toString()} tokens`,
    );
  }
  ...
  async function borrowTokens(amount, interestRate) {
    ...
    console.log(
      `Borrowing balance: ${await aaveLendingPool.getUserBorrowBalance(wallet.address).toString()} tokens`,
    );
  }
  ...
  async function tradeTokens(
    ...
    const inputTokenBalance = await tokenContract.balanceOf(
      wallet.address,
      inputToken,
    );
    ...
    await uniswapV2Router.swapExactTokens(
      inputAmount,
      outputAmount,
      inputToken,
      outputToken,
    ...
    console.log(
      `Output token balance: ${await tokenContract.balanceOf(wallet.address, outputToken).toString()} tokens`,
    );
  }
  ...
  async function yieldFarmTokens(amount, interestRate) {
    ...
    console.log(
      `Yield farming balance: ${awaitcompoundLendingPool.getUserBalance(wallet.address).toString()} tokens`,
    );
  }
  ...
  async function openCDP(amount, collateral) {
    ...
    console.log(
      `CDP balance: ${await makerDAOCDP.getUserBalance(wallet.address).toString()} tokens`,
    );
  }
  ...
  async function tradePerpetualSwap(amount, price) {
    ...
    console.log(
      `Margin balance: ${await tokenContract.balanceOf(wallet.address).toString()} tokens`,
    );
  }
