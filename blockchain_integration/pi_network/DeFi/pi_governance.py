...
class PIGovernance:
    ...
    def propose(self, proposal: str, description: str) -> int:
        ...
        tx_hash = self.contract.functions.propose(proposal, description).transact(
    ...
    def vote(self, proposal_id: int, vote: bool) -> bool:
        ...
        tx_hash = self.contract.functions.vote(proposal_id, vote).transact(
            {"from": self.web3.eth.accounts[0]}
        )
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
    ...
    def execute_proposal(self, proposal_id: int) -> bool:
        ...
        tx_hash = self.contract.functions.executeProposal(proposal_id).transact(
            {"from": self.web3.eth.accounts[0]}
        )
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
    ...
    def set_quorum(self, quorum: int) -> bool:
        ...
        tx_hash = self.contract.functions.setQuorum(quorum).transact(
            {"from": self.web3.eth.accounts[0]}
        )
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
    ...
    def set_voting_period(self, voting_period: int) -> bool:
        ...
        tx_hash = self.contract.functions.setVotingPeriod(voting_period).transact(
            {"from": self.web3.eth.accounts[0]}
        )
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
    ...
    def integrate_uniswap_v2(self, uniswap_v2_address: str) -> bool:
        ...
        tx_hash = self.contract.functions.integrateUniswapV2(
            uniswap_v2_address
    ...
    def integrate_pi_oracle(self, pi_oracle_address: str) -> bool:
        ...
        tx_hash = self.contract.functions.integratePiOracle(pi_oracle_address).transact(
            {"from": self.web3.eth.accounts[0]}
        )
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
