...
class PIOracle:
    ...
    def set_price(self, asset: str, price: int) -> bool:
        ...
        tx_hash = self.contract.functions.setPrice(asset, price).transact(
    ...
    def add_asset(self, asset: str) -> bool:
        ...
        tx_hash = self.contract.functions.addAsset(asset).transact(
            {"from": self.web3.eth.accounts[0]}
        )
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
    ...
    def remove_asset(self, asset: str) -> bool:
        ...
        tx_hash = self.contract.functions.removeAsset(asset).transact(
            {"from": self.web3.eth.accounts[0]}
        )
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
