...
class PISmartContract:
    ...
    def create_contract(self, bytecode: str, abi: list) -> bool:
        ...
        tx_hash = self.contract.functions.createContract(bytecode, abi).transact({'from': self.web3.eth.accounts[0]})
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1

    def call_contract_function(self, contract_address: str, function_name: str, args: list) -> any:
    ...
    def send_transaction_to_contract(self, contract_address: str, function_name: str, args: list) -> bool:
        # Send a transaction to a smart contract
        contract = self.web3.eth.contract(address=contract_address, abi=self.get_abi())
        tx_hash = contract.functions[function_name](*args).transact({'from': self.web3.eth.accounts[0]})
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
    ...
    def set_contract_parameters(self, parameters: dict) -> bool:
        ...
        tx_hash = self.contract.functions.setParameters(parameters).transact({'from': self.web3.eth.accounts[0]})
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
