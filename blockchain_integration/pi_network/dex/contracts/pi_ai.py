...
class PIAI:
    ...
    def create_ai_model(self, model_data: dict) -> bool:
        ...
        tx_hash = self.contract.functions.createAIModel(model_data).transact({'from': self.web3.eth.accounts[0]})
    ...
    def train_ai_model(self, model_id: int, training_data: dict) -> bool:
        # Train an AI model
        tx_hash = self.contract.functions.trainAIModel(model_id, training_data).transact({'from': self.web3.eth.accounts[0]})
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
    ...
    def set_ai_parameters(self, parameters: dict) -> bool:
        ...
        tx_hash = self.contract.functions.setAIParameters(parameters).transact({'from': self.web3.eth.accounts[0]})
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
