...
class PIDataMarketplace:
    ...
    def list_data(self, data: dict) -> bool:
        ...
        tx_hash = self.contract.functions.listData(data).transact({'from': self.web3.eth.accounts[0]})
    ...
    def buy_data(self, data_id: int) -> bool:
        ...
        tx_hash = self.contract.functions.buyData(data_id).transact({'from': self.web3.eth.accounts[0]})
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
    ...
    def set_marketplace_parameters(self, parameters: dict) -> bool:
        ...
        tx_hash = self.contract.functions.setMarketplaceParameters(parameters).transact({'from': self.web3.eth.accounts[0]})
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
