...
class PIIoT:
    ...
    def create_iot_device(self, device_data: dict) -> bool:
        ...
        tx_hash = self.contract.functions.createIoTDevice(device_data).transact({'from': self.web3.eth.accounts[0]})
    ...
    def send_iot_data(self, device_id: int, data: dict) -> bool:
        ...
        tx_hash = self.contract.functions.sendIoTData(device_id, data).transact({'from': self.web3.eth.accounts[0]})
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
    ...
    def set_iot_parameters(self, parameters: dict) -> bool:
        ...
        tx_hash = self.contract.functions.setIoTParameters(parameters).transact({'from': self.web3.eth.accounts[0]})
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
