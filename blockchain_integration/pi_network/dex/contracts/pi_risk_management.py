...
class PIRiskManagement:
    ...
    def manage_risk(self, asset: str, amount: int, risk_level: int) -> bool:
        ...
        tx_hash = self.contract.functions.manageRisk(asset, amount, risk_level).transact({'from': self.web3.eth.accounts[0]})
    ...
    def set_risk_parameters(self, asset: str, parameters: dict) -> bool:
        ...
        tx_hash = self.contract.functions.setRiskParameters(asset, parameters).transact({'from': self.web3.eth.accounts[0]})
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
