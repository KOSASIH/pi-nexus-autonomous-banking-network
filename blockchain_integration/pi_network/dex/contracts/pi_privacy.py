...
class PIPrivacy:
    ...
    def set_privacy_settings(self, settings: dict) -> bool:
        ...
        tx_hash = self.contract.functions.setPrivacySettings(settings).transact(
    ...
    def share_data(self, user: str, data: dict) -> bool:
        ...
        tx_hash = self.contract.functions.shareData(user, data).transact(
            {"from": self.web3.eth.accounts[0]}
        )
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
    ...
    def revoke_data_access(self, user: str) -> bool:
        ...
        tx_hash = self.contract.functions.revokeDataAccess(user).transact(
            {"from": self.web3.eth.accounts[0]}
        )
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
