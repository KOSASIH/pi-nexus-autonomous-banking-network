...
class PIExchange:
    ...
    def create_order(self, asset: str, amount: int, price: int) -> bool:
        ...
        tx_hash = self.contract.functions.createOrder(asset, amount, price).transact({'from': self.web3.eth.accounts[0]})
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1

    def cancel_order(self, order_id: int) -> bool:
        # Cancel an order
        tx_hash = self.contract.functions.cancelOrder(order_id).transact({'from': self.web3.eth.accounts[0]})
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1

    def fill_order(self, order_id: int) -> bool:
        # Fill an order
        tx_hash = self.contract.functions.fillOrder(order_id).transact({'from': self.web3.eth.accounts[0]})
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
    ...
    def set_exchange_parameters(self, parameters: dict) -> bool:
        ...
        tx_hash = self.contract.functions.setExchangeParameters(parameters).transact({'from': self.web3.eth.accounts[0]})
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
