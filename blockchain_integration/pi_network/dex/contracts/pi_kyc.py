...
class PIKYC:
    ...
    def perform_kyc(self, user: str, kyc_data: dict) -> bool:
        # Perform a KYC check
        tx_hash = self.contract.functions.performKYC(user, kyc_data).transact({'from': self.web3.eth.accounts[0]})
    ...
    def set_kyc_parameters(self, parameters: dict) -> bool:
        ...
        tx_hash = self.contract.functions.setKYCParameters(parameters).transact({'from': self.web3.eth.accounts[0]})
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
