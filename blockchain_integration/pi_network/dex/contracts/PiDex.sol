...
contract PiDex {
    ...
    // Hybrid order matching algorithm (FIFO + Proportional Fairness)
    ...
    function listToken(address token, uint256 decimals) public {
        // Create a new token listing
        TokenListing memory listing = TokenListing(token, tokenListings[token].listingId + 1, decimals);
    ...
    function placeOrder(address token, uint256 amount, uint256 price) public {
        // Create a new order
        Order memory order = Order(orders[token].length + 1, msg.sender, amount, price, block.timestamp);
