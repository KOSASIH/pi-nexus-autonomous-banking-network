...
class PINFT:
    ...
    def create_nft(self, metadata: dict) -> int:
        # Create a new NFT
        tx_hash = self.contract.functions.createNFT(metadata).transact(
    ...
    def transfer_nft(self, to: str, token_id: int) -> bool:
        ...
        tx_hash = self.contract.functions.transferNFT(to, token_id).transact(
            {"from": self.web3.eth.accounts[0]}
        )
    ...
    def set_nft_parameters(self, parameters: dict) -> bool:
        ...
        tx_hash = self.contract.functions.setNFTParameters(parameters).transact(
            {"from": self.web3.eth.accounts[0]}
        )
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
