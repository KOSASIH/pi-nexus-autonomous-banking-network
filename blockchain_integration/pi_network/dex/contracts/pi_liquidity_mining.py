...
class PILiquidityMining:
    ...
    def add_liquidity(self, asset: str, amount: int) -> bool:
        ...
        tx_hash = self.contract.functions.addLiquidity(asset, amount).transact({'from': self.web3.eth.accounts[0]})
    ...
    def remove_liquidity(self, asset: str, amount: int) -> bool:
        ...
        tx_hash = self.contract.functions.removeLiquidity(asset, amount).transact({'from': self.web3.eth.accounts[0]})
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
    ...
    def set_liquidity_parameters(self, asset: str, parameters: dict) -> bool:
        ...
        tx_hash = self.contract.functions.setLiquidityParameters(asset, parameters).transact({'from': self.web3.eth.accounts[0]})
        return self.web3.eth.waitForTransactionReceipt(tx_hash).status == 1
