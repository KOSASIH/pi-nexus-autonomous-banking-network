...
class PiNetworkDB:
    ...
    async def create_transaction(self, encrypted_data: bytes):
        async with self.pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("INSERT INTO transactions (data) VALUES (%s)", (encrypted_data,))
    ...
    async def get_transaction(self, transaction_id: int):
        async with self.pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("SELECT data FROM transactions WHERE id = %s", (transaction_id,))
    ...
    async def create_user(self, user_data: dict):
        async with self.pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("INSERT INTO users (data) VALUES (%s)", (json.dumps(user_data),))
...
if __name__ == '__main__':
    db = PiNetworkDB(host='localhost', user='root', password='password', db='pi_network')
