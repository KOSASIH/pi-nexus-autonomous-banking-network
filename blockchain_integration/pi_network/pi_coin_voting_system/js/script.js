...
function castVote(vote) {
  ...
  document.getElementById('vote-average').innerHTML =
    `Average vote: $${voteAverage.toFixed(2)}`;
  document.getElementById('vote-standard-deviation').innerHTML =
    `Standard deviation: $${voteStandardDeviation.toFixed(2)}`;
}
...
document.getElementById('vote').addEventListener('input', () => {
  ...
  if (vote < 0) {
    document
      .getElementById('vote')
      .setCustomValidity('Please enter a positive number.');
  } else {
