...
pi_nexus_contract = w3.eth.contract(
    address=to_checksum_address(PI_NEXUS_CONTRACT_ADDRESS), abi=pi_nexus_abi
)
...
def get_governance_proposals():
    ...
    return jsonify(
        [
            {"id": proposal[0], "description": proposal[1], "votes": proposal[2]}
...
def create_governance_proposal():
    ...
    tx_hash = pi_nexus_contract.functions.createGovernanceProposal(
        description
    ).transact({"from": api_account.address})
...
def vote_on_governance_proposal(proposal_id):
    ...
    tx_hash = pi_nexus_contract.functions.voteOnGovernanceProposal(
        proposal_id, vote
    ).transact({"from": api_account.address})
    return jsonify({"tx_hash": tx_hash})
...
def distribute_rewards():
    tx_hash = pi_nexus_contract.functions.distributeRewards().transact(
...
def add_liquidity():
    ...
    tx_hash = pi_nexus_contract.functions.addLiquidity(amount).transact(
        {"from": api_account.address}
    )
    return jsonify({"tx_hash": tx_hash})
...
def get_borrowing_requests():
    ...
    return jsonify(
        [{"address": request[0], "amount": request[1]} for request in requests]
    )
...
def request_borrowing():
    amount = request.json["amount"]
    tx_hash = pi_nexus_contract.functions.requestBorrowing(amount).transact(
        {"from": api_account.address}
    )
    return jsonify({"tx_hash": tx_hash})
...
def approve_borrowing_request(request_id):
    tx_hash = pi_nexus_contract.functions.approveBorrowingRequest(request_id).transact(
        {"from": api_account.address}
    )
    return jsonify({"tx_hash": tx_hash})
