...
describe("PiNetworkBridge", function () {
  ...
  describe("Deposit", function () {
    it("Should deposit Pi tokens", async function () {
      ...
      await piNetworkBridge.deposit("100000000000000000", { from: deployer.address });

      expect(await piToken.balanceOf(piNetworkBridge.address)).to.equal("100000000000000000");
    });
  });
  ...
  describe("Withdraw", function () {
    it("Should withdraw Pi tokens", async function () {
      ...
      await piNetworkBridge.withdraw("100000000000000000", { from: deployer.address });

      expect(await piToken.balanceOf(piNetworkBridge.address)).to.equal("0");
      expect(await piToken.balanceOf(deployer.address)).to.equal("100000000000000000");
    });
  });
});
