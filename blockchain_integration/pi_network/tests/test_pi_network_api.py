...
class TestPiNetworkAPI(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_create_transaction(self):
        ...
        async with self.client.post('http://localhost:8080/transactions', json=request) as response:
    ...
    async def test_get_transaction(self):
        transaction_id = 1
        async with self.client.get(f'http://localhost:8080/transactions/{transaction_id}') as response:
            self.assertEqual(response.status, 200)
            transaction_data = await response.json()
            self.assertEqual(transaction_data, {'amount': 10.99, 'ender': 'Alice', 'eceiver': 'Bob'})

    async def test_create_user(self):
        request = {'username': 'Alice', 'password': 'password123'}
        async with self.client.post('http://localhost:8080/users', json=request) as response:
            self.assertEqual(response.status, 201)
    ...
    async def test_get_user(self):
        user_id = 1
        async with self.client.get(f'http://localhost:8080/users/{user_id}') as response:
            self.assertEqual(response.status, 200)
            user_data = await response.json()
            self.assertEqual(user_data, {'username': 'Alice', 'password': 'password123'})

    async def test_process_transaction_data(self):
        transaction_data = {'amount': 10.99, 'ender': 'Alice', 'eceiver': 'Bob'}
        async with self.client.post('http://localhost:8080/transactions/process', json=transaction_data) as response:
            self.assertEqual(response.status, 200)
            processed_data = await response.json()
            self.assertEqual(processed_data, {'amount': 10.99, 'ender_id': 1, 'eceiver_id': 2})
