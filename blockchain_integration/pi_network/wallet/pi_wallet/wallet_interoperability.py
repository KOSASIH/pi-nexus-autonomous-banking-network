...
def generate_recommendations(balance, network):
    ...
    if balance > 0:
        recommendations.append(
            "Consider sending a portion of your balance to another wallet for increased security"
        )
    if network == "ethereum":
        recommendations.append(
            "Consider transferring to a Bitcoin wallet for diversification"
        )
    elif network == "bitcoin":
        recommendations.append(
            "Consider transferring to an Ethereum wallet for increased usability"
        )
