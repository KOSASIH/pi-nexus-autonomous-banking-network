...
class InteroperabilityProtocol:
    ...
    def send_transaction(self, transaction):
        ...
        main_chain_response = requests.post(
            self.main_chain_url, json=encrypted_transaction
        )
        main_chain_tx_hash = main_chain_response.json()["tx_hash"]

        # Send transaction to side chain
        side_chain_response = requests.post(
            self.side_chain_url, json=encrypted_transaction
        )
...
if __name__ == "__main__":
    ...
    main_chain_tx_hash, side_chain_tx_hash = interoperability_protocol.send_transaction(
        transaction
    )
