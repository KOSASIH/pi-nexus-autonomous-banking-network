...
class MultiSigWallet:
    ...
    def generate_keys(self):
        for _ in range(self.num_signatures_total):
            private_key = ecdsa.SigningKey.from_secret_exponent(123, curve=ecdsa.SECP256k1)
    ...
    def create_multisig_address(self):
        ...
        multisig_address = hashlib.sha256(b''.join([pk.to_string() for pk in self.public_keys])).hexdigest()
    ...
    def authorize_transaction(self, transaction, signatures):
        ...
        for signature in signatures:
            public_key_index = self.public_keys.index(VerifyingKey.from_string(signature))
