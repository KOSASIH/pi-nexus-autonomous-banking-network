...
class WalletAdvancedFeatures:
    ...
    def portfolio_optimization(self):
        ...
        def portfolio_optimization_problem(weights):
            ...
            portfolio_risk = np.sqrt(np.dot(weights.T, np.dot(self.wallet_data['covariance_matrix'], weights)))
        ...
        # Initialize weights
        init_weights = np.array([1.0 / len(self.wallet_data['assets'])] * len(self.wallet_data['assets']))

        # Optimize portfolio
        result = minimize(portfolio_optimization_problem, init_weights, method='SLSQP', constraints=constraints)
