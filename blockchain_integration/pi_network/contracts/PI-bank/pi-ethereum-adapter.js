...
class PiEthereumAdapter {
  ...
  async sendPi(fromPiAddress, toPiAddress, amount) {
    const txData = await this.pi.createSendTransaction(fromPiAddress, toPiAddress, amount);
  ...
  async sendEth(fromEthAddress, toEthAddress, amount) {
    ...
    const signedTx = await this.ethereum.eth.accounts.signTransaction(txData, fromEthAddress);
    const txHash = await this.ethereum.eth.sendSignedTransaction(signedTx.rawTransaction);
    return txHash;
  }
}
