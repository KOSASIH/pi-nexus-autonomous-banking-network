...
class PiNetworkAPI:
    ...
    async def create_transaction(self, request: web.Request):
        try:
            ...
            transaction_id = await self.pi_network_core.create_transaction(
                encrypted_data
            )
            return web.Response(
                text=json.dumps({"transaction_id": transaction_id}), status=201
            )
    ...
    async def get_transaction(self, request: web.Request):
        try:
            ...
            transaction_data = await self.pi_network_core.get_transaction(
                transaction_id
            )
