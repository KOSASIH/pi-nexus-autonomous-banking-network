...
EXCHANGES = [
    ...
    {'name': 'Indodax', 'api_url': 'https://api.indodax.com/v2'},
...
def update_pi_coin_stable_value_on_exchange(exchange):
    ...
    response = requests.get(f"{exchange['api_url']}/ticker/price", params={'symbol': PI_COIN_SYMBOL})
    ...
    if abs(price_diff) > Decimal('0.01'):
        response = requests.post(f"{exchange['api_url']}/ticker/update", json={'symbol': PI_COIN_SYMBOL, 'price': STABLE_VALUE})
        if response.status_code != 200:
            print(f"Error updating stable value on {exchange['name']}: {response.text}")
            return
        
        print(f"Updated stable value of PI coin on {exchange['name']} to {STABLE_VALUE}!")
