...
EXCHANGES = [
    ...
    {
        "name": "Huobi",
        "api_url": "https://api.huobi.pro/market",
...
def list_pi_coin_on_exchange(exchange):
    ...
    response = requests.post(
        f"{exchange['api_url']}/ticker/new",
        json={"symbol": market_pair, "quoteAsset": "USDT"},
    )
    ...
    response = requests.post(
        ...
        json={"symbol": market_pair, "price": STABLE_VALUE},
    )
    if response.status_code != 200:
...
def main():
    for exchange in EXCHANGES:
        ...
        send_ws_message(
            ws, {"method": "SUBSCRIBE", "params": [f"{PI_COIN_SYMBOL}@trade"], "id": 1}
        )
