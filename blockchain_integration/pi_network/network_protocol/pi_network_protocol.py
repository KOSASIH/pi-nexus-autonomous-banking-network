...
class PINetworkProtocol:
    ...
    async def establish_connection(self, node_id, ip, port):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f"https://{ip}:{port}/connect", 
                                    headers={"Node-ID": self.node_id},
                                    data={"public_key": self.public_key}) as resp:
                if resp.status == 200:
    ...
    async def send_message(self, node_id, message):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f"https://{self.network_topology[node_id][0]}:{self.network_topology[node_id][1]}/message", 
                                    headers={"Node-ID": self.node_id},
                                    data={"message": encrypted_message}) as resp:
                if resp.status == 200:
    ...
    async def receive_message(self):
        ...
        while True:
            ...
            print(f"Received message from node {message['node_id']}: {decrypted_message}")

    def _encrypt_message(self, message):
        # AES-256-GCM encryption
        cipher = Cipher(algorithms.AES(self.private_key), modes.GCM(self.private_key), backend=default_backend())
