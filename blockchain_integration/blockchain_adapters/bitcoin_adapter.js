...
class BitcoinAdapter {
  ...
  async sendTransaction(fromAddress, toAddress, value) {
    ...
    const address = bitcoin.payments.p2pkh({ pubkey: publicKey, network: this.network }).address;
  ...
  async broadcastTransaction(rawTransaction) {
    ...
    const receipt = await client.sendCommand('sendrawtransaction', [rawTransaction]);
    return receipt;
  }
}
