...
class MobileAppService {
  ...
  async transfer(user, recipient, amount) {
    ...
    // Transfer the assets
    const transaction = await this.mobileAppUtils.transfer(
      user,
      recipient,
      amount,
    );
  ...
  async subscribeToPushNotifications(user) {
    ...
    const subscription =
      await this.mobileAppUtils.subscribeToPushNotifications(user);
  ...
  async unsubscribeFromPushNotifications(user) {
    ...
    const success =
      await this.mobileAppUtils.unsubscribeFromPushNotifications(user);
