...
class Exchange:
    ...
    def get_rates(self) -> List[Dict[str, str]]:
        response = requests.get(f'https://{self.name}.com/api/rates', headers={'API-KEY': self.api_key})
    ...
    def get_balances(self) -> List[Dict[str, str]]:
        response = requests.get(f'https://{self.name}.com/api/balances', headers={'API-KEY': self.api_key})
        if response.status_code == 200:
    ...
    def execute_trade(self, symbol: str, amount: float) -> bool:
        response = requests.post(f'https://{self.name}.com/api/trade', headers={'API-KEY': self.api_key}, json={'symbol': symbol, 'amount': amount})
        if response.status_code == 200:
            return True
        else:
...
class AutonomousGlobalExchangesConnector:
    def __init__(self, config: Config):
        self.exchanges = [Exchange(name, api_key) for name, api_key in config.config['exchanges'].items()]
    ...
    def execute_trade(self, exchange_name: str, symbol: str, amount: float) -> bool:
        exchange = next((exchange for exchange in self.exchanges if exchange.name == exchange_name), None)
        if exchange:
