...
def get_accounts(bank_id):
    ...
    if bank:
        ...
        for account in bank.accounts.values():
            accounts.append({'account_id': account.account_id, 'account_type': account.account_type, 'balance': account.balance})
    ...
    if bank:
        ...
        if account:
            ...
            for transaction in account.transactions:
                transactions.append({'transaction_id': transaction.transaction_id, 'amount': transaction.amount, 'attributes': transaction.attributes})
