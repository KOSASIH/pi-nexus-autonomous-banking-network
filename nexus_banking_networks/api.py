...
from nexus_banking_network import NexusAccount, NexusTransaction, NexusLedger
...
class AccountResource(Resource):
    def get(self, account_id):
        account = next((acc for acc in ledger.accounts if acc.account_id == account_id), None)
        if account:
