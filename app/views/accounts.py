...
def accounts_new():
    if request.method == 'POST':
        ...
        account = Account(username=username, email=email, password=password, balance=balance, currency_id=currency_id)
...
def accounts_transactions(account_id):
    ...
    return render_template('accounts/transactions.html', account=account, transactions=transactions)
...
def accounts_transactions_new(account_id):
    ...
    if request.method == 'POST':
        ...
        transaction = Transaction(sender_id=account_id, receiver_id=receiver_id, amount=amount, currency_id=currency_id)
    ...
    currencies = Currency.query.all()
    return render_template('accounts/transactions_new.html', account=account, receivers=receivers, currencies=currencies)

@accounts.route('/accounts/<int:account_id>/transactions/<int:transaction_id>/delete', methods=['POST'])
