...
from fnex.models import Account, Transaction, User
...
class TestFineXModels(unittest.TestCase):
    def setUp(self):
        ...
        self.account = Account.objects.create(
            user=self.user, account_number="123456789", balance=1000
        )
        self.transaction = Transaction.objects.create(
            account=self.account, amount=-50, description="Test transaction"
        )
