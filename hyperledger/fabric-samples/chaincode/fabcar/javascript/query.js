...
async function main() {
  try {
    ...
    if (!userExists) {
      console.log(
        'An identity forthe user "user1" does not exist in the wallet',
      );
    ...
    const carAsBytes = await contract.evaluateTransaction(
      "QueryCar",
      carNumber,
    );
    const car = JSON.parse(carAsBytes.toString());
    console.log(
      `Car ${carNumber} has colour ${car.colour}, make ${car.make} and model ${car.model}`,
    );
