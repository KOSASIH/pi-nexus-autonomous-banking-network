...
async function authenticateUser (requiredAttributes) {
  try {
    ...
    const verified = uport.JWT.verify(
      payload,
      signature,
      web3.currentProvider.provider.options.rpcUrl
    )
...
async function addCredential (address, credential) {
  try {
    const response = await uportConnect.identity.addCredential(
      address,
      credential
    )
...
async function removeCredential (address, credentialId) {
  try {
    const response = await uportConnect.identity.removeCredential(
      address,
      credentialId
    )
...
async function getIdentity (identityAddress) {
  try {
